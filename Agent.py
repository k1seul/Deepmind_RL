...
class Agent:
    def __init__(self, args, writer: SummaryWriter) -> None:
        ...
        self.env = wrap_deepmind(env, episode_life=True, clip_rewards=True, frame_stack=False)
    ...
    def _init_simple_network(self, args): 
        ...
        if self.double_net:
            if self.dual_net:
                self.network = DualDQNSimple(n_actions=self.n_actions, FRAME_STACK=self.frame_stack) 
                self.target_net = DualDQNSimple(n_actions=self.n_actions, FRAME_STACK=self.frame_stack)
            else:
                self.network = DQNSimple(n_actions=self.n_actions, FRAME_STACK=self.frame_stack) 
                self.target_net = DQNSimple(n_actions=self.n_actions, FRAME_STACK=self.frame_stack)
    ...
    def _init_experience_replay(self, args):
        ...
        if self.per:
            self.mem = ExperienceMemoryBufferPER(maxlen=args.buffer_size, state_shape=[84,84], n_step=self.n_steps, FRAME_STACK=self.frame_stack)
            self.loss = MSELoss(reduce='none')
        else:
            self.mem = ExperienceMemoryBuffer(maxlen=args.buffer_size, state_shape=[84,84], n_step=self.n_steps, FRAME_STACK=self.frame_stack)
            self.loss = MSELoss() 

    def _init_experience_replay_simple(self, args):
        if self.per:
            self.mem = ExperienceMemoryBufferPER(maxlen=args.buffer_size, state_shape=[1,4], n_step=self.n_steps, FRAME_STACK=self.frame_stack)
            self.loss = MSELoss(reduce='none')
        else:
            self.mem = ExperienceMemoryBuffer(maxlen=args.buffer_size, state_shape=[1, 4], n_step=self.n_steps, FRAME_STACK=self.frame_stack)
            self.loss = MSELoss() 
    ...
    def train(self, writer):
        ...
        while t < self.total_frames:
            ...
            if episode_num % 100 == 0 and episode_num > 0:
                ...
                save_gif_to_tensorboard(np.stack(rendered_imgs), writer=writer, global_step = episode_num, tag='gif')
            ...
            if episode_num % 10 == 0:
                ...
                writer.add_scalar('data/steps', round(t/self.total_frames * 100, 1), episode_num )
                writer.add_scalar('data/episode_length', episode_length, episode_num )
                writer.add_scalar('data/td_error', td_loss_total / episode_length, episode_num )     
                writer.add_scalar('data/avg_q', avg_q_values_total / episode_length, episode_num)

            print(f"Episode Number: {episode_num}, rewards: {total_rewards}, epsilon: {self.epsilon}")
            episode_num += 1
    ...
    def update(self, samples):
        ...
        Q_target_n_next_states = (self.target_net if self.double_net else self.network)(next_states[-1]).detach()

        # Calculate Q_current_states and Q_max_next_actions just once
        Q_current_states = self.network(states[0])[torch.arange(actions[0].size(0)), actions[0]]
        Q_next_states = self.network(next_states[-1])
        ...
        reward_to_add = Q_target_n_next_states[range(Q_max_next_actions.shape[0]), Q_max_next_actions]
    ...
    def save_network(self, step):
        ...
        torch.save(self.network.state_dict(), f'data/{self.env_name}/{self.network_name}_{step}.pt')
