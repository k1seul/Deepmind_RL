...
class Agent:
    def __init__(self, writer,env_name = "ALE/Breakout-v5", device='cpu') -> None:
        self.mem = ExperienceMemoryBuffer(maxlen=100_000)
        self.env = make_env(env_name=env_name)
        self.n_actions = self.env.action_space.n
        self.network = DuelDQN(n_actions=self.n_actions, device=device)
        self.device = device
        self.epsilon = 1.0
        self.epsilon_final = 0.01
        self.epsilon_decay_rate = 0.9 / 1_000_000
        self.loss = MSELoss() 
        self.optim = torch.optim.Adam(self.network.parameters(), lr=0.00025, betas = [0.9, 0.999])
        self.gamma = 0.99
        ...
        self.avg_q_value = 0 
        self.update_freq = 4
        self.target_net = DuelDQN(n_actions=self.n_actions, device=device)
    ...
    def action(self, state, exploit = False):
        if random.random() < self.epsilon and not exploit:
            ...
            adv_values, val = self.network(obs.reshape(1, *obs.shape))
            action = np.argmax(adv_values.detach().cpu().numpy())
    ...
    def update(self, samples):
        ...
        advantages_current, values_current = self.network(states)
        Q_current_states = values_current + (advantages_current - torch.mean(advantages_current, 1, keepdim=True))

        advantages_next, values_next = self.network(next_states)
        advantages_next = advantages_next.detach()
        values_next = values_next.detach()
        Q_next_states = values_next + (advantages_next - torch.mean(advantages_next, 1, keepdim=True))

        Q_max_next_actions = torch.argmax(Q_next_states, axis=1)

        tgt_adv_next_states, tgt_value_next_states = self.target_net(next_states)
        tgt_adv_next_states = tgt_adv_next_states.detach()
        tgt_value_next_states = tgt_value_next_states.detach() 
        tgt_Q_next_states = tgt_value_next_states + (tgt_adv_next_states- torch.mean(tgt_adv_next_states, 1, keepdim=True))
        # import pdb; pdb.set_trace()
        y_i = rewards + self.gamma * tgt_Q_next_states[range(Q_max_next_actions.shape[0]), Q_max_next_actions]* (1 - dones)
