...
class ExperienceMemoryBuffer:
    ...
    def sample(self, batch_size = 32):
        ...
        states = np.stack([np.stack([self.select_stacked_states(i + j) for i in indices]) for j in range(self.n_step)]) 
        ...
        next_states = np.stack([np.stack([self.select_stacked_states(i+j+1) for i in indices]) for j in range(self.n_step)]) 
...
class ExperienceMemoryBufferPER:
    ### Numpy implementation
    def __init__(self, maxlen = 10_000, state_shape = (84, 84), n_step=1, FRAME_STACK=4, beta_start=0.4):
        self.capacity = maxlen
        self.states = np.zeros((self.capacity, *state_shape), dtype=np.uint8)
    ...
    def beta_anneal(self):
        self.beta = min(self.beta + (self.beta_end - self.beta)/self.beta_anneal_episode, self.beta_end)
    ...
    def sample(self, batch_size = 32):
        random_indecies, priorities, indices = self.per_heap.sample(batch_size=batch_size)
    ...
    def update_td_error(self, td_errors):
        for i in range(32): 
            self.per_heap.update(self.sampled_indices[i], td_errors[i]**self.per_heap.alpha)
        # print(sum(td_errors))
        # import pdb; pdb.set_trace()
...
class TestExperienceMemoryBuffer(unittest.TestCase):
    ...
    def setUp(self):
        self.buffer = ExperienceMemoryBuffer(maxlen=10, state_shape=(84, 84), n_step=2, FRAME_STACK=4)
...
class TestExperienceMemoryBufferPER(unittest.TestCase):
    ...
    def setUp(self):
        self.buffer = ExperienceMemoryBufferPER(maxlen=10, state_shape=(84, 84), n_step=2, FRAME_STACK=4)
        self.state = np.ones((84, 84), dtype=np.uint8)
        self.action = 1
        self.reward = 1.0
        self.done = False
