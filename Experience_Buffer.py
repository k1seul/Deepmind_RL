...
class TestExperienceMemoryBuffer(unittest.TestCase):
    ...
    def buffer_overflow(self):
        for i in range(25):
            self.buffer.add_experience(self.state, i, self.reward, self.done)

        data = self.buffer.sample(10)
        print(*data)
...
if __name__ == "__main__":
    # unittest.main()
    mem = ExperienceMemoryBuffer(maxlen=10, state_shape=(1,2))
    for i in range(25):
        mem.add_experience((1,2), i, i, False)
    print(mem.sample(9)[1])
