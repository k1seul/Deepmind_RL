...
class PriorityHeap:
    ...
    def sort(self):
        self.perheap[:self.position] = self.perheap[:self.position][np.argsort(self.perheap[:self.position, 0])[::-1]]
    ...
    def random_batch_indicies(self):
        ...
        random_indecies += [random.randint(random_batch_start[i-1], random_batch_start[i] )  for i in range(1, self.k-1)] 
...
class SumTree:
    def __init__(self, capacity):
        ...
        self.alpha = 0.6
        self.batch_size = 32 
        self.p_ranges = [(i/self.batch_size, (i+1)/self.batch_size) for i in range(self.batch_size)]
        self.max = 1
...
class TestSumTree(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertTrue(np.array_equal(self.sum_tree.tree, np.zeros(2 * self.capacity - 1)))
        self.assertTrue(np.array_equal(self.sum_tree.data, np.zeros(self.capacity, dtype=np.uint32)))
    ...
    def test_add(self):
        ...
        # import pdb; pdb.set_trace()
        self.assertAlmostEqual(self.sum_tree.tree[self.capacity], priority_values[1]  ** self.sum_tree.alpha)
        
        self.assertEqual(self.sum_tree.total_sum, sum(priority_values ** self.sum_tree.alpha) ,"total sum is equal")
        self.assertEqual(self.sum_tree.n, 0, "Index should wrap around to 0 after filling capacity")
    ...
    def test_update(self):
        ...
        self.assertEqual(self.sum_tree.total_sum, self.sum_tree.tree[0], "Total sum should be updated")
        self.assertAlmostEqual(self.sum_tree.total_sum, old_sum + 2 ** self.sum_tree.alpha - 1)
