...
class DistributionalDQN(nn.Module):
    ...
    def __init__(self, args, n_actions=4):
        ...
        if args.noisy_net:
            self.fc1 = NoisyLayer(7 * 7 * 64, 512, device=args.device)
            self.fc2 = NoisyLayer(512, n_actions * N_ATOMS, device=args.device)
        else:
            self.fc1 = nn.Linear(7 * 7 * 64, 512, device=args.device)
            self.fc2 = nn.Linear(512, n_actions * N_ATOMS, device=args.device)
        self.device = args.device
        sups = torch.arange(Vmin, Vmax + DELTA_Z, DELTA_Z).to(self.device)
    ...
    def forward(self, x):
        ...
        x = F.relu(x) 
        out = self.fc2(x)
        return out.view(batch_size, -1, 51)
    ...
    def apply_softmax(self, x):
        return self.softmax(x.view(-1, 51)).view(x.size())

def weight_clip(model, clip_value):
    """
    Clips the weights of the model parameters to be within the range [-clip_value, clip_value].
