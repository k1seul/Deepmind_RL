...
def run_combinations():
    ...
    combinations = list(itertools.product(
        n_step_values, frame_stack_values, double_net_values, dual_net_values, per_values
    ))
    ...
    for n_step, frame_stack, double_net, dual_net, per in combinations:
        ...
        tasks.append((n_step, frame_stack, double_net, dual_net, per, network_name, total_frames))
...
def run_main_logic(args):
    ...
    """
    print(f"Executing main logic for {args.network_name} with TOTAL_FRAMES={args.total_frames}")
    # Your main code goes here
    env_name = args.env_name
    now = datetime.now()
    day_time = now.strftime("%m%d%h")
    ...
    torch.save(agent.network.state_dict(), f'data/{args.env_name}/{args.network_name}.pt')
    writer.close() 
    time.sleep(1)
